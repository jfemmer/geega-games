-- Replaces the always-mocked admin Analytics (Overview/Trends pages) with
-- real aggregation over orders, order_items, inventory_items,
-- inventory_movements, customers, newsletter_subscribers, and campaigns.
-- Same SECURITY DEFINER + is_staff() gating pattern as admin_search_inventory
-- (staff-callable directly from the browser client, no service-role
-- round-trip needed for read-only aggregation).
--
-- Both functions return jsonb shaped to match the existing
-- AnalyticsRepository TypeScript interface exactly (OverviewMetrics /
-- TrendMetrics in src/admin/types/index.ts) so the live repository is a
-- straight decode, no shape translation.
...
revoke all on function public.admin_analytics_trends(text) from public, anon, authenticated;
grant execute on function public.admin_analytics_trends(text) to authenticated, service_role;
