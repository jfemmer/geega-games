-- public.search_inventory only ever excluded status='archived'. A
-- status='reserved' inventory line (the whole-line hold, distinct from a
-- per-unit row in inventory_reservations) was NOT excluded, so it could be
-- returned to storefront search/browse as purchasable — contradicting
-- inventory_public (the view the rest of the storefront reads), which
-- strictly requires status='active', and the documented domain rule
-- (src/admin/types/index.ts ListingStatus: "reserved rows are hidden from
-- the storefront but retained"). No 'reserved' rows exist in production
-- today, so this hasn't visibly manifested yet, but it's a live landmine.
--
-- Fix: always require status='active', with p_include_archived as the only
-- escape hatch (and only for 'archived', never 'reserved'). Strictly
-- narrower than before for 'reserved' only — every other row's inclusion is
-- unchanged. Same signature/return shape/sort/pagination.
...
AS $function$
  ...
  filtered as (
    ...
    where
      (
        b.status = 'active'
        or (p_include_archived is true and b.status = 'archived')
      )
      and (
